...
SELECT games, loadingtime, duration, left FROM players WHERE name=?
SELECT * FROM one;
...
CREATE TABLE config ( name TEXT NOT NULL PRIMARY KEY, value TEXT NOT NULL )
SELECT * FROM <table2>;
SELECT 1 FROM %Q.'%q_segments' WHERE blockid=? AND block IS NULL
SELECT %s FROM %s T WHERE T.%Q=?
SELECT size FROM %Q.'%q_docsize' WHERE docid=?
SELECT COUNT(*) FROM bans WHERE server=?
...
CREATE TABLE x(input, token, start, end, position)
SELECT * FROM admins WHERE server=? AND name=?
SELECT * FROM \"%w\".\"%w\", \"%w\".\"%w\" WHERE %s AND (%z)
UPDATE t1 SET c = usa  WHERE a = 4
CREATE TABLE bans ( id INTEGER PRIMARY KEY, server TEXT NOT NULL, name TEXT NOT NULL, date TEXT NOT NULL, admin TEXT NOT NULL, reason TEXT )
SELECT %s, rbu_control FROM '%q' 
SELECT %s, rbu_control FROM %s.'rbu_tmp_%q' ORDER BY %s%s
SELECT name FROM sqlite_master WHERE rootpage = ?
UPDATE t1 SET c = rbu_delta WHERE a = 4
SELECT nodeno FROM '%q'.'%q_rowid' WHERE rowid = :1
SELECT COUNT(*) FROM admins WHERE server=?
...
SELECT * FROM two;
UPDATE players SET games=?, loadingtime=?, duration=?, left=?  WHERE name=?
...
SELECT country FROM iptocountry WHERE ip1<=? AND ip2>=?
...
SELECT * FROM rootadmins WHERE server=? AND name=?
...
SELECT value FROM config WHERE name="schema_number")", (void**)&Statement);
CREATE TABLE <tbl>(input, token, start, end, position)
CREATE TABLE t1(col INTEGER)
CREATE INDEX Ex2 ON Ex1(c3,c1);
SELECT value FROM %Q.'%q_stat' WHERE id=?
UPDATE players SET dotas=?, wins=?, losses=?, kills=?, deaths=?, creepkills=?, creepdenies=?, assists=?, neutralkills=?, towerkills=?, raxkills=?, courierkills=?  WHERE name=?
