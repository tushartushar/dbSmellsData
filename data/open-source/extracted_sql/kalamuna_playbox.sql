...
SELECT name, filename, info FROM {system} WHERE type = 'module' ORDER BY name ASC
...
SELECT rid, name FROM {role} WHERE rid IN (:selected)
...
SELECT * FROM {user} WHERE uid = :uid', array(':uid' => $conf['uid']))->fetchObject();
SELECT tid FROM {taxonomy_term_data} WHERE name = :name
SELECT uid FROM {user} WHEREuid = :uid', array(':uid' => $uid))->fetchObject();
...
SELECT vid FROM {fieldable_panels_panes} WHERE fpid = :fpid
...
SELECT * FROM {node_access} WHERE (nid = 0 OR nid = :nid) AND grant_view = 1', array(':nid' => $item->nid));
...
SELECT entity_id, field_user_picture_fid FROM {field_data_field_user_picture} WHERE entity_type = 'user'
...
SELECT 1 FROM {" . $table . "}
SELECT * FROM {linkit_profiles} ORDER BY weight DESC
...
SELECT count(*) FROM {taxonomy_term_data} WHERE vid = :vid
...
SELECT entity_type, entity_id, pathauto FROM {pathauto}");
...
SELECT uid FROM {user} WHERE LOWER(name) = LOWER(:name)', array(':name' => $uid))->fetchObject();
...
SELECT filename FROM {registry} WHERE name = :name AND type = :type
...
SELECT count(*) FROM {users} WHERE uid > :uid", array(':uid' => 0))->fetchField() : db_query("SELECT count(*) FROM {users} u INNER JOIN {users_roles} r on u.uid = r.uid WHERE r.rid = :rid
...
SELECT count(*) FROM {node} WHERE type = :type and status = 1
...
SELECT count(*) FROM {menu_links} WHERE menu_name = :menu_name
...
SELECT name,type,info FROM {system} WHERE type = :type AND status = :status
...
SELECT name FROM {variable} WHERE name LIKE 'defaultcontent_%'
SELECT data, item_id FROM {queue} q WHERE expire = 0 AND name = :name ORDER BY created ASC', 0, 1, array(':name' => $this->name))->fetchObject();
